...
for i = 1:num_stocks
    ...
    try
        ...
        % For ADVANC: D:\CatBoost\SET50\ADVANC\backtest_results_iter0.xlsx
        % For AOT: D:\CatBoost\SET50\AOT\backtest_results_iter0.xlsx
        % For WHA: D:\CatBoost\SET50\WHA\backtest_results_iter0.xlsx
        ...
        try
            ...
            if ~any(strcmp(sheets, 'ModelMetrics'))
                fprintf('  Warning: ModelMetrics sheet not found. Available sheets: %s\n', strjoin(sheets, ', '));
                rmse_values(i) = NaN;
            else
                % Read from ModelMetrics sheet (as shown in the user's table)
                ...
                if isempty(model_data)
                    ...
                    % Extract RMSE from cell C2 (Out-of-Sample RMSE value)
                    % Based on the table structure: Metric | In-Sample | Out-of-Sample
                    % RMSE is in row 2, Out-of-Sample is column 3 (C)
                    if size(model_data, 1) >= 2 && size(model_data, 2) >= 3
                        % Get the Out-of-Sample RMSE value (row 2, column 3)
                        rmse_cell_value = model_data{2, 3}; % Row 2, Column 3 (C2)
                        
                        if isnumeric(rmse_cell_value) && ~isnan(rmse_cell_value)
                            rmse_values(i) = rmse_cell_value;
                            fprintf('  RMSE value from cell C2 (Out-of-Sample): %.6f\n', rmse_values(i));
                        else
                            fprintf('  Warning: RMSE value in cell C2 is not numeric: %s\n', num2str(rmse_cell_value));
                            rmse_values(i) = NaN;
                        end
                    else
                        fprintf('  Warning: ModelMetrics sheet does not have expected structure\n');
                        fprintf('  Sheet size: %dx%d, expected at least 2x3\n', size(model_data, 1), size(model_data, 2));
                        rmse_values(i) = NaN;
                    end
                end
            end
        ...
        try
            % First check if the sheet exists
            [~, sheets] = xlsfinfo(file_path);
            if ~any(strcmp(sheets, 'Summary'))
                fprintf('  Warning: Summary sheet not found. Available sheets: %s\n', strjoin(sheets, ', '));
                sharpe_values(i) = NaN;
            else
                % Read from Summary sheet (as shown in the user's table)
                ...
                if isempty(summary_data)
                    ...
                    % Extract Sharpe ratio from cell B5 (Value column, row 5)
                    % Based on the table structure: Metric | Value
                    % Sharpe Ratio is in row 5, Value is column 2 (B)
                    if size(summary_data, 1) >= 5 && size(summary_data, 2) >= 2
                        % Get the Sharpe ratio value (row 5, column 2)
                        sharpe_cell_value = summary_data{5, 2}; % Row 5, Column 2 (B5)
                        
                        if isnumeric(sharpe_cell_value) && ~isnan(sharpe_cell_value)
                            sharpe_values(i) = sharpe_cell_value;
                            fprintf('  Sharpe ratio from cell B5: %.6f\n', sharpe_values(i));
                        else
                            fprintf('  Warning: Sharpe ratio value in cell B5 is not numeric: %s\n', num2str(sharpe_cell_value));
                            sharpe_values(i) = NaN;
                        end
                    else
                        fprintf('  Warning: Summary sheet does not have expected structure\n');
                        fprintf('  Sheet size: %dx%d, expected at least 5x2\n', size(summary_data, 1), size(summary_data, 2));
                        sharpe_values(i) = NaN;
                    end
                end
            end
...
try
    writetable(results_table, 'SET50_analysis_results.csv');
    fprintf('Results saved to SET50_analysis_results.csv\n');
catch ME
